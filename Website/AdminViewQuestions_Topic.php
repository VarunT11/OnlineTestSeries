...
<br>
Enter the Name of Topic - <input type=text name=topic>
<br>
...
if(isset($_POST["bt"])){
	$qtyp=$_POST["qtyp"];$subject=$_POST["subject"];$topic=$_POST["topic"];
	$_SESSION["subject"]=$subject;$_SESSION["qtyp"]=$qtyp;$_SESSION["topic"]=$topic;
		if(!isset($qtyp) or !isset($subject))
			die("Please Select all the Fields");
	$conn=new mysqli("localhost","root","","$subject");
		$q="select * from $qtyp where Topic='$topic'";
		$q1="select * from $qtyp";
		$select1=$conn->query($q1);
		$select=$conn->query($q);
		$_SESSION["totalq"]=$select1->num_rows;
		if($select->num_rows>0){
			$TotalQuestions=$select->num_rows;
			echo "Subject - $subject &nbsp&nbsp Type - $qtyp<br>Topic - $topic &nbsp&nbsp Total Questions - $TotalQuestions<br>
